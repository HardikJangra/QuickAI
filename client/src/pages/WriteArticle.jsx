...
const WriteArticle = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try{
      setLoading(true)
      const prompt = `Write a detailed article about ${input} in ${selectedLength.length} `
      const {data}= await axios.post('http://localhost:3000/api/ai/generate-article', {prompt, length: selectedLength.length},{headers: {Authorization: `Bearer ${await getToken()}`}})
      if(data.success){
